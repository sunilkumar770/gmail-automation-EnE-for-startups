...
async function POST(req) {
  ...
  let event;
  try {
    event = new import_svix.Webhook(signingSecret).verify(rawBody, svixHeaders);
  } catch (err) {
  ...
  console.info(`[resend-webhook] verified ${eventType} for ${event?.data?.email_id ?? "?"}`);
  if (!HANDLED_TYPES.has(eventType)) {
    return next_server_shim_exports.NextResponse.json({ received: true, handled: false, reason: "event type not subscribed" });
  }
  const rpc = await callRpc("handle_resend_webhook_event", { p_event: event });
  if (!rpc.ok) {
    console.error(`[resend-webhook] RPC failed (${rpc.status}):`, rpc.error);
    return next_server_shim_exports.NextResponse.json({ error: "processing failed" }, { status: 500 });
  }
  return next_server_shim_exports.NextResponse.json({ received: true, ...rpc.data });
}
async function GET() {
